...
public interface ServisRepository extends CrudRepository<Servis, Long> {
    @Query(value = "select * from servis i where i.active = true ", nativeQuery = true)
    List<Servis> findAllActive();
    @Query(value = "select * from servis i where i.active = true AND i.fk_unit = :unitId", nativeQuery = true)
